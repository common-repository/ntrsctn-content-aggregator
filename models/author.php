...
class NTRSCTN_JSON_API_Author {
  ...
  // Note:
  //   NTRSCTN_JSON_API_Author objects can include additional values by using the
  //   author_meta query var.
  
  function NTRSCTN_JSON_API_Author($id = null) {
    if ($id) {
      $this->id = (int) $id;
    } else {
  ...
  function set_author_meta() {
    ...
    if (!$ntrsctn_json_api->query->author_meta) {
      return;
    }
    ...
    $vars = explode(',', $ntrsctn_json_api->query->author_meta);
