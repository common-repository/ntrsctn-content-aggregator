...
class NTRSCTN_JSON_API_Post {
  
  // Note:
  //   NTRSCTN_JSON_API_Post objects must be instantiated within The Loop.
  ...
  var $content_read_more; // String (modified by read_more query var)
  var $content;		// String (full content not modified by read_more query var)
  var $excerpt;         // String
  ...
  function NTRSCTN_JSON_API_Post($wp_post = null) {
    if (!empty($wp_post)) {
  ...
  function save($values = null) {
    global $ntrsctn_json_api, $user_ID;
    ...
    if (!empty($values['author'])) {
      $author = $ntrsctn_json_api->introspector->get_author_by_login($values['author']);
    ...
    if (isset($values['categories'])) {
      ...
      foreach ($categories as $category_slug) {
        ...
        $category = $ntrsctn_json_api->introspector->get_category_by_slug($category_slug);
    ...
    if (!empty($_FILES['attachment'])) {
      ...
      $this->attachments[] = new NTRSCTN_JSON_API_Attachment($attachment_id);
  ...
  function import_wp_object($wp_post) {
    ...
    $date_format = $ntrsctn_json_api->query->date_format;
  ...
  function set_value($key, $value) {
    ...
    if ($ntrsctn_json_api->include_value($key)) {
      $this->$key = $value;
    } else {
      unset($this->$key);
    }
  ...
  function set_content_value() {
    global $ntrsctn_json_api, $more;

    if ($ntrsctn_json_api->include_value('content')) {
      $content = get_the_content($ntrsctn_json_api->query->read_more);
      ...
      unset($this->content);
      unset($this->content_read_more);
    }
  }
  
  function set_categories_value() {
    global $ntrsctn_json_api;
    if ($ntrsctn_json_api->include_value('categories')) {
      $this->categories = array();
      if ($wp_categories = get_the_category($this->id)) {
        foreach ($wp_categories as $wp_category) {
          $category = new NTRSCTN_JSON_API_Category($wp_category);
  ...
  function set_tags_value() {
    global $ntrsctn_json_api;
    if ($ntrsctn_json_api->include_value('tags')) {
      $this->tags = array();
      if ($wp_tags = get_the_tags($this->id)) {
        foreach ($wp_tags as $wp_tag) {
          $this->tags[] = new NTRSCTN_JSON_API_Tag($wp_tag);
        }
      }
    } else {
      unset($this->tags);
  ...
  function set_author_value($author_id) {
    global $ntrsctn_json_api;
    if ($ntrsctn_json_api->include_value('author')) {
      $this->author = new NTRSCTN_JSON_API_Author($author_id);
    } else {
      unset($this->author);
    }
  }
  ...
  function set_comments_value() {
    global $ntrsctn_json_api;
    if ($ntrsctn_json_api->include_value('comments')) {
      $this->comments = $ntrsctn_json_api->introspector->get_comments($this->id);
    } else {
      unset($this->comments);
    }
  }
  ...
  function set_attachments_value() {
    global $ntrsctn_json_api;
    if ($ntrsctn_json_api->include_value('attachments')) {
      $this->attachments = $ntrsctn_json_api->introspector->get_attachments($this->id);
    } else {
  ...
  function set_thumbnail_value() {
    global $ntrsctn_json_api;
    if (!$ntrsctn_json_api->include_value('thumbnail') ||
  ...
  function set_custom_fields_value() {
    global $ntrsctn_json_api;
    if ($ntrsctn_json_api->include_value('custom_fields') &&
        $ntrsctn_json_api->query->custom_fields) {
      $keys = explode(',', $ntrsctn_json_api->query->custom_fields);
  ...
  function get_thumbnail_size() {
    global $ntrsctn_json_api;
    if ($ntrsctn_json_api->query->thumbnail_size) {
      return $ntrsctn_json_api->query->thumbnail_size;
