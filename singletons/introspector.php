...
class NTRSCTN_JSON_API_Introspector {
  ...
  public function get_posts($query = false, $wp_posts = false) {
    ...
    while (have_posts()) {
      ...
      if ($wp_posts) {
        ...
        $output[] = new NTRSCTN_JSON_API_Post($post);
      }
    }
    return $output;
  }
  ...
  public function get_current_category() {
    ...
    extract($ntrsctn_json_api->query->get(array('id', 'slug', 'category_id', 'category_slug')));
    if ($id || $category_id) {
      if (!$id) {
        $id = $category_id;
      }
  ...
  public function get_current_tag() {
    global $ntrsctn_json_api;
    extract($ntrsctn_json_api->query->get(array('id', 'slug', 'tag_id', 'tag_slug')));
    if ($id || $tag_id) {
      if (!$id) {
        $id = $tag_id;
      }
  ...
  public function get_current_author() {
    global $ntrsctn_json_api;
    extract($ntrsctn_json_api->query->get(array('id', 'slug', 'author_id', 'author_slug')));
    if ($id || $author_id) {
      if (!$id) {
        $id = $author_id;
      }
  ...
  public function get_author_by_id($id) {
    ...
    return new NTRSCTN_JSON_API_Author($id);
  }
  ...
  public function get_comments($post_id) {
    ...
    foreach ($wp_comments as $wp_comment) {
      $comments[] = new NTRSCTN_JSON_API_Comment($wp_comment);
    }
    return $comments;
  }
  ...
  public function get_attachments($post_id) {
    ...
    if (!empty($wp_attachments)) {
      foreach ($wp_attachments as $wp_attachment) {
        $attachments[] = new NTRSCTN_JSON_API_Attachment($wp_attachment);
      }
    }
    return $attachments;
  }
  ...
  public function attach_child_posts(&$post) {
    ...
    foreach ($wp_children as $wp_post) {
      $post->children[] = new NTRSCTN_JSON_API_Post($wp_post);
    }
  ...
  protected function get_category_object($wp_category) {
    ...
    return new NTRSCTN_JSON_API_Category($wp_category);
  }
  ...
  protected function get_tag_object($wp_tag) {
    ...
    return new NTRSCTN_JSON_API_Tag($wp_tag);
  }
  ...
  protected function set_posts_query($query = false) {
    global $ntrsctn_json_api, $wp_query;
    ...
    if ($ntrsctn_json_api->query->page) {
      $query['paged'] = $ntrsctn_json_api->query->page;
    }
    
    if ($ntrsctn_json_api->query->count) {
      $query['posts_per_page'] = $ntrsctn_json_api->query->count;
    }
    
    if ($ntrsctn_json_api->query->post_type) {
      $query['post_type'] = $ntrsctn_json_api->query->post_type;
    }
    
    if ($ntrsctn_json_api->query->post_status == 'deleted') {
