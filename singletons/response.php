...
class NTRSCTN_JSON_API_Response {
  
  function setup() {
    global $ntrsctn_json_api;
    $this->include_values = array();
    $this->exclude_values = array();
    if ($ntrsctn_json_api->query->include) {
      $this->include_values = explode(',', $ntrsctn_json_api->query->include);
    }
    // Props to ikesyo for submitting a fix!
    if ($ntrsctn_json_api->query->exclude) {
      $this->exclude_values = explode(',', $ntrsctn_json_api->query->exclude);
  ...
  function get_json($data, $status = 'ok') {
    ...
    $data = apply_filters('ntrsctn_json_api_encode', $data);
  ...
  function respond($result, $status = 'ok') {
    ...
    if ($ntrsctn_json_api->query->dev || !empty($_REQUEST['dev'])) {
    ...
    } else if ($ntrsctn_json_api->query->redirect) {
      $url = $this->add_status_query_var($ntrsctn_json_api->query->redirect, $status);
      wp_redirect($url);
    } else if ($ntrsctn_json_api->query->callback) {
      // Run a JSONP-style callback with the result
      $this->callback($ntrsctn_json_api->query->callback, $json);
    } else {
      // Output the result
      $this->output($json);
    }
